...
int main(){
    ...
    char string[256]; 
    int res;
    t_log = fopen("time2.txt","w+");

    t1 = clock();

    t2 = clock();
    system("tpm2_verifysignature -k 0x81010005 -g 0x000B -m input.txt -s output1.txt -t ticket2.txt");
    system("echo $? >> result.txt");

    verifykey_t= (double)(clock() - t2);
    fprintf(t_log, "verify %.6fms\n",(verifykey_t /CLOCKS_PER_SEC)*1000);
    ...
    t3 = clock();
    system("tpm2_sign -k 0x81010005 -g 0x000B -m key.txt -s output1.txt");
    signkey_t= (double)(clock() - t3);
    fprintf(t_log, "sign %.6fms\n",(signkey_t /CLOCKS_PER_SEC)*1000);
